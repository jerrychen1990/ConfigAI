...
class CLSTokenClassifyModel(AbstractTextClassifyModel):
    ...
    #             self.label_num, name="classifier", activation=classify_activation,
    ...
    #     with self.get_scope():
    #         classify_output = Input(shape=(self.label_num,) if self.multi_label else (), name='classify_output',
    #                                 dtype=tf.int32)
    ...
    # @discard_kwarg
    # @log_cost_time
    # def _post_infer(self, pred_tensors, show_detail=False, threshold=.5) -> List[LabelOrLabels]:
